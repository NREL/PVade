...
The ``Gmsh_code`` folder contains standalone geometry and meshing prototype
...
The YAML files under ``input/`` (for example ``panels3d.yaml`` or
``turbinflow_duramat_case_study.yaml``) are the main supported workflow for
running PVade simulations.

By contrast, files in ``Gmsh_code`` are utility/prototype scripts that can help
...
.. code-block:: bash
   ...
   conda run -n PVade python Gmsh_code/demo/demo_elipsoidal_shape_triangle_surface.py

or

.. code-block:: bash

   conda run -n PVade python Gmsh_code/fake_terrain/fake_terrain_shape_triangle_surface.py
