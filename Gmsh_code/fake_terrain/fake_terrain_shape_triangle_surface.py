...
interpolate_func = scipy.interpolate.LinearNDInterpolator(list(zip(imported_x,imported_y)),imported_z)  # interpolate the data to z = f(x,y)
...
x_array,y_array = np.meshgrid(x_array,y_array)        # y = [[0,0...],[].....[100,100,100...]], x = [[0,...100],[0,...100],[]...[0,...100]], x[i], y[j] coordinates of points on jth row.
...
for I in range(ny):
    for J in range(nx-1):
        gmsh.model.occ.addLine(pt_tag+I*nx+J, pt_tag+I*nx+J+1, tag = line_hor+(nx-1)*I+J)
        if pt_tag+I*nx+J in pt_tag_on_xy_plane and pt_tag+I*nx+J+1 in pt_tag_on_xy_plane:
...
for J in range(nx):
    for I in range(ny-1):
        gmsh.model.occ.addLine(pt_tag+I*nx+J, pt_tag+(I+1)*nx+J, tag = line_ver+(ny-1)*J+I)
        if pt_tag+I*nx+J in pt_tag_on_xy_plane and pt_tag+(I+1)*nx+J in pt_tag_on_xy_plane:
...
for I in range(ny-1):
    for J in range(nx-1):
        gmsh.model.occ.addLine(pt_tag+I*nx+J, pt_tag+(I+1)*nx+J+1, tag = line_dia+(nx-1)*I+J)
        if pt_tag+I*nx+J in pt_tag_on_xy_plane and pt_tag+(I+1)*nx+J+1 in pt_tag_on_xy_plane:
...
for I in range(ny-1):
    for J in range(nx-1):
        gmsh.model.occ.addCurveLoop([line_hor+(nx-1)*I+J, line_ver+(ny-1)*(J+1)+I, -line_dia-(nx-1)*I-J], tag = cl_ter+I*(2*(nx-1))+2*J+0)
        gmsh.model.occ.addCurveLoop([line_ver+(ny-1)*J+I, line_hor+(nx-1)*(I+1)+J, -line_dia-(nx-1)*I-J], tag = cl_ter+I*(2*(nx-1))+2*J+1)
...
for I in range(ny-1):
    for J in range(nx-1):
        gmsh.model.occ.addPlaneSurface([cl_ter+I*(2*(nx-1))+2*J+0], tag = s_ter+I*(2*(nx-1))+2*J+0)
        if line_hor+(nx-1)*I+J in line_on_xy_plane and line_ver+(ny-1)*(J+1)+I in line_on_xy_plane and line_dia+(nx-1)*I+J in line_on_xy_plane:
            s_ter_on_xy_plane.append(s_ter+I*(2*(nx-1))+2*J+0)
        gmsh.model.occ.addPlaneSurface([cl_ter+I*(2*(nx-1))+2*J+1], tag = s_ter+I*(2*(nx-1))+2*J+1)
        if line_ver+(ny-1)*J+I in line_on_xy_plane and line_hor+(nx-1)*(I+1)+J in line_on_xy_plane and line_dia+(nx-1)*I+J in line_on_xy_plane:
...
bottom_list.extend(list(range(line_ver+(ny-1)*(nx-1), line_ver+(ny-1)*(nx-1)+ny-2+1)))
bottom_list.extend(list(range(-line_hor-(nx-1)*(ny-1)-nx+2,-line_hor-(nx-1)*(ny-1)+1)))
...
aaa = gmsh.model.occ.cut([(2, s_bottom)], remove_list, tag = s_bottom+1, removeObject = True, removeTool = True)
...
surface_list.extend(list(range(s_ter, s_ter+(ny-2)*(2*(nx-1))+2*(nx-2)+2)))
