...
for I in range(1, nx):
    for J in range(0, ny):
        gmsh.model.occ.addLine(p_center+(ny+1)*(I-1)+J, p_center+(ny+1)*(I-1)+J+1, tag = line_ver+ny*(I-1)+J)
...
for J in range(ny):
    gmsh.model.occ.addCurveLoop([line_left+J, line_ver+J, -line_left-J-1], tag = cl_left+J)
...
for I in range(1,nx-1):
    for J in range(2*ny+1):
        if (J%2==0):
           gmsh.model.occ.addLine(p_center+(ny+1)*(I-1)+int(J/2), p_center+(ny+1)*(I)+int(J/2), tag = line_center+(2*ny+1)*(I-1)+J)
        else:
           gmsh.model.occ.addLine(p_center+(ny+1)*(I-1)+int((J+1)/2), p_center+(ny+1)*(I)+int((J-1)/2), tag = line_center+(2*ny+1)*(I-1)+J)

# create line loop at center surfaces
cl_center_left = line_center+(2*ny+1)*(nx-2-1)+2*ny+1
for I in range(1,nx-1):
    for J in range(ny):
        gmsh.model.occ.addCurveLoop([line_ver+ny*(I-1)+J, line_center+(2*ny+1)*(I-1)+2*J+1, -(line_center+(2*ny+1)*(I-1)+2*J)], tag = cl_center_left+ny*(I-1)+J)


s_center_left = cl_center_left+ny*(nx-2-1)+ny
for I in range(1, nx-1):
    for J in range(ny):
        gmsh.model.occ.addPlaneSurface([cl_center_left+ny*(I-1)+J], tag = s_center_left+ny*(I-1)+J)

cl_center_right = s_center_left+ny*(nx-2-1)+ny
for I in range(1, nx-1):
    for J in range(ny):
        gmsh.model.occ.addCurveLoop([line_center+(2*ny+1)*(I-1)+2*J+1, line_ver+ny*(I)+J, -(line_center+(2*ny+1)*(I-1)+2*J+2)], tag = cl_center_right+ny*(I-1)+J)

s_center_right = cl_center_right+ny*(nx-2-1)+ny
for I in range(1, nx-1):
    for J in range(ny):
        gmsh.model.occ.addPlaneSurface([cl_center_right+ny*(I-1)+J], tag = s_center_right+ny*(I-1)+J)

# create lines at most right surface

line_right = s_center_right+ny*(nx-2-1)+ny
for J in range(ny+1):
    gmsh.model.occ.addLine(p_right, p_center+(ny+1)*(nx-2)+J, tag = line_right+J)

# create curve loops at most right surface

cl_right = line_right+ny+1
for J in range(ny):
        gmsh.model.occ.addCurveLoop([line_right+J, line_ver+ny*(nx-2)+J, -line_right-J-1], tag = cl_right+J)
...
bottom_list.extend(list(range(line_center,line_center+(2*ny+1)*(nx-2),2*ny+1)))
bottom_list.append(-line_right)
bottom_list.append(line_right+ny)
bottom_list.extend(list(range(-(line_center+(2*ny+1)*(nx-3)+2*ny),-(line_center+2*ny)+2*ny+1,2*ny+1)))
...
surface_list.extend(list(range(s_center_right,s_center_right+ny*(nx-2-1)+ny)))
