...
class ModalAnalysis:
    ...
    def build_forms(self, domain, params, structure):
        ...
        # # self.k_form = k(self.avg(self.u_old, self.u, self.alpha_f), self.u_)
        # # self.m_form = m(self.avg(self.a_old, a_new, self.alpha_m), self.u_)
        
        # Convenience functions
        def sigma(u):
            return structure.λ * ufl.nabla_div(u) * ufl.Identity(len(u)) \
                + 2*structure.μ*epsilon(u)
           
        def epsilon(u):
            return ufl.sym(ufl.grad(u))
        
        self.k_form = dolfinx.fem.form(ufl.inner(sigma(self.u), epsilon(self.u_))*ufl.dx)
        self.m_form = dolfinx.fem.form(structure.rho*ufl.dot(self.u, self.u_)*ufl.dx)

        # self.k_form = dolfinx.fem.form(k(self.avg(self.u_old, self.u, self.alpha_f), self.u_))
        # self.m_form = dolfinx.fem.form(m(self.avg(self.a_old, a_new, self.alpha_m), self.u_))

        
    def _assemble_system(self, params, structure):
        ...
        self.K = dolfinx.fem.petsc.assemble_matrix(self.k_form, bcs=self.bc) #, diagonal=62831)
        self.M = dolfinx.fem.petsc.assemble_matrix(self.m_form, bcs=self.bc) #, diagonal=1/62831)
        self.K.assemble()
        self.M.assemble()
        ...
        if self.first_call_to_solver:
            ...
            self.N_eig = 6
            self.eigensolver = SLEPc.EPS().create(MPI.COMM_WORLD)
            self.eigensolver.setDimensions(self.N_eig)
            self.eigensolver.setProblemType(SLEPc.EPS.ProblemType.GHEP)
            st = SLEPc.ST().create(MPI.COMM_WORLD)
            st.setType(SLEPc.ST.Type.SINVERT)
            st.setShift(0.1)
            st.setFromOptions()
            self.eigensolver.setST(st)
            self.eigensolver.setOperators(self.K, self.M)
            self.eigensolver.setFromOptions()
    ...
    def solve(self, params, dataIO, structure,domain):
        # def σ(v):
        #     """Return an expression for the stress σ given a displacement field"""
        ...
        if self.first_call_to_solver:
            ...
            self._assemble_system(params, structure)

        # Compute eigenvalue-eigenvector pairs
        self.eigensolver.solve()
        evs = self.eigensolver.getConverged()
        vr, vi = self.K.getVecs()
        u_output = dolfinx.fem.Function(self.V)
        ...
        if evs > 0:
            with dolfinx.io.XDMFFile(MPI.COMM_WORLD, "eigenvectors.xdmf", "w") as xdmf:
                xdmf.write_mesh(domain.structure.msh)
                for i in range (min(self.N_eig, evs)):
                    l = self.eigensolver.getEigenpair(i, vr, vi)
